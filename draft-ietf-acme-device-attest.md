...
{{!RFC8555}} section 7.4 mandates that "The CSR MUST indicate the exact same set of requested identifiers as the initial newOrder request". However, there are some environments where the Server requires validation of the identifier but does not include the identifier in certificates due to privacy concerns. This document updates {{!RFC8555}} to specify that the CSR SHOULD indicate the exact same set of requested identifiers of types `permanent-identifier` and `hardware-module` ({{hardware-module}}) as the initial newOrder request (identifiers of other types MUST be indicated in the CSR per {{!RFC8555}} section 7.4). To support privacy-preserving certificates, Clients MAY omit identifiers of types `permanent-identifier` and `hardware-module` in the certificate signing request (CSR). Similarly, if the Server wishes to issue privacy-preserving certificates, it MAY reject CSRs containing a PermanentIdentifier or HardwareModuleName in the subjectAltName extension. See the {{privacy-considerations}} for more information.
...
{{!RFC8555}} section 7.4 mandates that "The CSR MUST indicate the exact same set of requested identifiers as the initial newOrder request". This document updates {{!RFC8555}} to specify that the CSR SHOULD indicate the exact same set of requested identifiers of types `permanent-identifier` ({{permanent-identifier}}) and `hardware-module` as the initial newOrder request (identifiers of other types MUST be indicated in the CSR per {{!RFC8555}} section 7.4). To support privacy-preserving certificates, Clients MAY omit identifiers of types `permanent-identifier` and `hardware-module` in the certificate signing request (CSR). Similarly, if the Server wishes to issue privacy-preserving certificates, it MAY reject CSRs containing a PermanentIdentifier or HardwareModuleName in the subjectAltName extension. See the {{privacy-considerations}} for more information.
