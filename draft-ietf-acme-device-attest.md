...
Using ACME and device attestation to issue client certificates for enterprise PKI is a common use case. The following variances to the ACME specification are described in this document:
...
# Device Attestation Challenge {#device-attestation-challenge}
...
2. Verify that *attToBeSigned* contains the key authorization or the token, according to the construction required by the attestation format, and that the value matches what the Server stored.
...
Although this document focuses guidance on implementing new identifier
types and a challenge for certificate issuance using ACME, it does not
define a new protocol, a protocol extension, or an architecture.
